...
public class EnemyHunk : Enemy
{
    ...
    private void Run()
    {
        ...
        if(Math.Abs(transform.position.x - movePos.position.x) > 0.1)
        {
            ...
            transform.position = Vector2.MoveTowards(transform.position, movePos.position, runSpeed * Time.deltaTime);
    ...
    {
        Vector2 randomPos = new Vector2(UnityEngine.Random.Range(leftMovePos.position.x, rightMovePos.position.x), transform.position.y);
